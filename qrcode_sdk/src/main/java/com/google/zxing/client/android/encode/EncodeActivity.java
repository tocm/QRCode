...
public final class EncodeActivity extends Activity {
    ...
    private void share() {
        ...
        String displayContents = encoder.getContents();
        ...
        File barcodeFile = new File(barcodesRoot, makeBarcodeFileName(displayContents) + ".png");
        ...
        intent.putExtra(Intent.EXTRA_TEXT, displayContents);
    ...
    protected void onResume() {
        ...
        try {
            ...
            //Modify by Andy 20160713
//      if (intent.getBooleanExtra(Intents.Encode.SHOW_CONTENTS, true)) {
//        contents.setText(displayContents);
...
//      }
     ...
     * @return
     */
    private BarcodeEntity createBarcodeEntityData() {
        try {
            BarcodeEntity grb = new BarcodeEntity();
            grb.setContents(qrCodeEncoder.getDisplayContents());
            grb.setBarcodeBuf(Utils.BitmapToBytes(qrCodeEncoder.encodeAsBitmap()));
            return grb;
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * add by Andy.chen
     */
    private void trigerResultIntent() {
        LogUtils.d(TAG, "=== barcode result intent ====");
        if (qrCodeEncoder != null) {
            LogUtils.d(TAG, "barcode result intent send finish intent ");
            try {
                Intent cbIntent = new Intent();
                Bundle bundle = new Bundle();
                bundle.putParcelable(BarcodeEntity.CB_RESULT_KEY,
                        createBarcodeEntityData());
                cbIntent.putExtras(bundle);
                setResult(RESULT_OK, cbIntent);
                this.finish();
                LogUtils.d(TAG, "createBarcodeEntityData setResult ");
            } catch (Exception e) {
                e.printStackTrace();
            }

            LogUtils.d(TAG, "barcode result intent send finish end ");
        }
